...
export function CalculadoraROI({
  ...
  const calcularCenario = (scenario: Scenario) => {
    ...
    // 5. ROI do Estetia
    const custoEstetiaMensal = numVendedores * CUSTO_SIRIUS_MENSAL
    const roi60Dias = (ganho60Dias / (custoEstetiaMensal * 2)) * 100
    const paybackDias = custoEstetiaMensal > 0 && ganhoMensal > 0 ? (custoEstetiaMensal / ganhoMensal) * 30 : 0
        ...
        <CardContent className="space-y-8 pt-6">
                    ...
                    <div className="grid gap-6 sm:grid-cols-4">
                      <div>
                        ...
                        <p className="text-xl font-bold text-green-700 dark:text-green-400">
                          {formatCurrency(result.custoEstetiaMensal)}/mês
                        </p>
                      ...
                      <div>
                        ...
                        <p className="text-xl font-bold text-green-700 dark:text-green-400">
                          {formatCurrency(result.ganhoAnual - (result.custoEstetiaMensal * 12))}
                        </p>
                      </div>
                    </div>
                  </div>
          ...
          <div className="pt-4">
            <Button
              ...
              size="lg"
              className="w-full h-14 text-lg font-bold bg-gradient-to-r from-[#0a1f3d] to-[#0a1f3d] hover:from-indigo-700 hover:to-indigo-800 shadow-xl shadow-indigo-500/30 transition-all hover:scale-[1.02] active:scale-[0.98]"
            >
              {ctaText}
