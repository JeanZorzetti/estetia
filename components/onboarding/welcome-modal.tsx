    ...
    <Dialog open={open} onOpenChange={(open) => !open && !isLoading && onClose()}>
      <DialogContent className="w-[55vw] sm:max-w-[55vw] max-w-[55vw]!" onPointerDownOutside={(e) => isLoading && e.preventDefault()}>
        ...
        <div className="grid gap-4 py-6 sm:grid-cols-3">
          ...
          <Card
            className={`relative p-6 cursor-pointer transition-all hover:shadow-lg hover:scale-[1.02] border-2 ${
              selectedChoice === 'demo'
                ? 'border-[#0a1f3d] bg-[#eef2f9] dark:bg-indigo-950/20'
                : 'border-zinc-200 dark:border-zinc-800 hover:border-indigo-300'
            }`}
            onClick={() => !isLoading && handleChoice('demo')}
          >
            {/* Recommended Badge */}
            <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-1 rounded-full bg-gradient-to-r from-[#1a3560] to-[#1a3560] text-white text-xs font-bold">
              Recomendado ✨
            </div>

            <div className="space-y-4 pt-2">
              <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-[#1a3560] to-[#1a3560] flex items-center justify-center mx-auto">
              ...
              <Button
                className="w-full bg-gradient-to-r from-[#0a1f3d] to-[#0a1f3d] hover:from-indigo-700 hover:to-purple-700"
                disabled={isLoading}
              >
        ...
        <div className="text-center text-xs text-muted-foreground pt-2 border-t">
          <p>💡 Dica: Escolha a demonstração para ver o poder do Estetia em ação!</p>
        </div>
      </DialogContent>
    </Dialog>
