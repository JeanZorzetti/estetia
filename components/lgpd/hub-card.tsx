...
interface HubCardProps {
  ...
  href: string
  kpis: KpiItem[]
  colorClass: 'CONSENTIMENTOS' | 'AUDIT_LOG' | 'EXPORTACAO' | 'ANONIMIZACAO' | string
}
...
export function HubCard({ icon: Icon, title, description, href, kpis, colorClass }: HubCardProps) {
  ...
  let typeKey: keyof typeof CARD_CONFIG = 'CONSENTIMENTOS'
  if (colorClass.includes('emerald')) typeKey = 'CONSENTIMENTOS'
  else if (colorClass.includes('blue')) typeKey = 'AUDIT_LOG'
  else if (colorClass.includes('violet')) typeKey = 'EXPORTACAO'
  else if (colorClass.includes('amber')) typeKey = 'ANONIMIZACAO'
