...
export function NewPostModal({ open, onClose, onCreated, preselectedDate }: Props) {
  ...
  async function handleStep1Next() {
    ...
    if (source === 'ai') {
      ...
      setGenerating(true)
      try {
        const imagePrompt = `Instagram post for Estetia CRM software. Dark background with purple and blue gradient. A glowing 4-pointed star logo top left with text "SIRIUS CRM". Bold white text in Portuguese about: "${topic}". Professional SaaS design. Minimalist modern style.`
        const res = await fetch('/api/instagram/generate', {
          ...
          body: JSON.stringify({
            type,
            imagePrompt,
            caption: `✨ ${topic}\n\nCom o Estetia CRM, corretores têm tudo que precisam para vender mais.\n\n👉 estetiacrm.com.br`,
            hashtags: '#EstetiaCRM #CRMImobiliario #CorretorDeImoveis #Automacao #VendasImobiliarias',
            altText: `Post do Estetia CRM sobre ${topic}`,
            slides: type === 'carousel' ? [topic, 'Problema', 'Solução', 'Resultado', '➡️ estetiacrm.com.br'] : [],
            draftOnly: true,
          }),
        })
            ...
            <div key={s} className="flex items-center gap-0 flex-1">
              <div className={`flex items-center gap-1.5 ${step >= s ? 'text-purple-400' : 'text-muted-foreground/40'}`}>
                <div className={`w-5 h-5 rounded-full border text-[10px] font-bold flex items-center justify-center transition-colors
                  ${step === s ? 'border-purple-500 bg-[#489fb5]/15 text-purple-400' : step > s ? 'border-purple-500/50 bg-[#489fb5]/8' : 'border-border'}
                `}>
                  {s}
                </div>
                    ...
                    {TYPE_OPTIONS.map(opt => (
                      <button
                        ...
                        className={`rounded-xl border p-3 text-left transition-all
                          ${type === opt.value ? 'border-purple-500 bg-[#489fb5]/8' : 'border-border hover:border-purple-500/40'}
                        `}
                      >
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <button
                      ...
                      className={`rounded-xl border p-3 flex items-center gap-2 transition-all
                        ${source === 'ai' ? 'border-purple-500 bg-[#489fb5]/8' : 'border-border hover:border-purple-500/40'}
                      `}
                    >
                    ...
                    <button
                      ...
                      className={`rounded-xl border p-3 flex items-center gap-2 transition-all
                        ${source === 'upload' ? 'border-purple-500 bg-[#489fb5]/8' : 'border-border hover:border-purple-500/40'}
                      `}
                    >
