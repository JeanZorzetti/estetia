...
function LeadCard({ lead }: { lead: NearbyLead }) {
  ...
  return (
    <div className="flex items-start gap-3 rounded-2xl border border-border/60 bg-card p-4 shadow-sm transition-all active:scale-[0.99]">
      {/* Avatar */}
      <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-indigo-100 text-indigo-700 dark:bg-[#0a1f3d]/30 dark:text-[#c5a059]">
      ...
      <div className="min-w-0 flex-1">
        ...
        {lead.lastDealTitle && (
          <p className="mt-0.5 truncate text-[11px] text-[#0a1f3d]/80 dark:text-[#489fb5]/80">
            {lead.lastDealTitle}
          </p>
        )}
    ...
    <div className="mx-auto max-w-lg space-y-4 pb-4">
        ...
        {RADIUS_OPTIONS.map((opt) => (
          <button
            ...
            className={cn(
              ...
              radius === opt.value
                ? 'border-indigo-600 bg-[#0a1f3d] text-white shadow-sm'
      ...
      {(status === 'locating' || status === 'loading') && (
        <div className="flex flex-col items-center gap-3 py-16">
          <Loader2 className="h-8 w-8 animate-spin text-[#0a1f3d]" />
          <p className="text-sm text-muted-foreground">
