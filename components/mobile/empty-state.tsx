...
export function MobileEmptyState({ icon: Icon, title, description, action, className }: MobileEmptyStateProps) {
  return (
    <div className={cn('flex flex-col items-center justify-center gap-3 py-16 text-center px-6', className)}>
      <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-br from-[#1a3560]/10 to-[#1a3560]/5 ring-1 ring-indigo-500/10">
        <Icon className="h-6 w-6 text-[#1a3560]/60" />
      </div>
      ...
      {action && (
        <button
          ...
          className="mt-1 rounded-xl bg-[#1a3560] px-5 py-2.5 text-sm font-semibold text-white shadow-sm shadow-indigo-500/20 transition-all active:scale-95"
        >
          {action.label}
        </button>
      )}
