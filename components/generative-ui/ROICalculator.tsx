 ...
 *
 * Demonstrates the value of Estetia CRM by calculating:
 * - Monthly savings
 * - Annual ROI
 * - Payback period
 *
...
export function ROICalculator({
  ...
  const [withEstetia, setWithEstetia] = useState(scenario.withEstetia)
  const [saved, setSaved] = useState(false)

  // Calculate derived values
  const monthlySavings = currentCost - withEstetia
  const annualSavings = monthlySavings * 12
  const paybackPeriod = withEstetia > 0 ? withEstetia / monthlySavings : 0
  ...
  }, [currentCost, withEstetia, monthlySavings, annualSavings, paybackPeriod, roiPercentage, industry, onInteraction])
  ...
  }, [currentCost, withEstetia, monthlySavings, annualSavings, onInteraction])
      ...
      <Card className="border-2 border-primary/20 shadow-lg">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <CardDescription>
                  Veja quanto você pode economizar com Estetia CRM
                </CardDescription>
              </div>
            </div>
            {industry && (
        ...
        <CardContent className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="withEstetia" className="flex items-center gap-2">
                <TrendingUp className="h-4 w-4 text-green-600" />
                Com Estetia CRM (Mensal)
              </Label>
              <div className="relative">
                <DollarSign className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
                <Input
                  ...
                  type="number"
                  value={withEstetia}
                  onChange={(e) => setWithEstetia(Number(e.target.value))}
                  className="pl-9"
                  min={0}
                />
              </div>
