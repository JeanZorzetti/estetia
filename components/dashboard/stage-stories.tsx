        ...
        return (
          <button
            ...
            className={cn(
              ...
              isActive
                ? 'border border-[#0a1f3d]/30 bg-[#1a3560]/5 shadow-sm'
          ...
          >
            ...
            <span className={cn(
              ...
              isActive ? 'text-[#0a1f3d] dark:text-[#489fb5]' : 'text-muted-foreground',
            )}>
              {stage.name}
            </span>
            {totalValue > 0 && (
              <span className={cn('text-[10px] font-medium', isActive ? 'text-[#1a3560]' : 'text-muted-foreground/70')}>
                {totalValue >= 1000
                  ? `R$ ${(totalValue / 1000).toFixed(0)}k`
                  : `R$ ${totalValue.toLocaleString('pt-BR')}`}
              </span>
            )}
            <span className={cn('text-[10px]', isActive ? 'text-[#489fb5]' : 'text-muted-foreground/50')}>
              ({stage.deals.length})
            </span>
          </button>
        )
