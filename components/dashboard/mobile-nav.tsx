...
function MobileNavLink({ item, pathname, onClose }: { item: NavItem; pathname: string; onClose: () => void }) {
  ...
  return (
    ...
    >
      ...
      <Icon className={cn("h-4 w-4 z-10 transition-colors", isActive ? "text-[#0a1f3d] dark:text-[#c5a059]" : "text-zinc-500 group-hover:text-zinc-700 dark:group-hover:text-zinc-300")} />
          ...
          <div className="flex h-16 items-center justify-between px-6 border-b border-border">
            <Link href="/dashboard" className="flex items-center gap-3" onClick={close}>
              <div className="relative w-8 h-8">
                <Logo variant="icon" width={32} height={32} className="object-contain" />
              </div>
              <div className="flex flex-col">
                <span className="text-sm font-bold text-[#0a1f3d] dark:text-[#c5a059]">Estetia</span>
