...
export function ProntuarioClient({ patient, records, anamneses, treatments, consentLogs, canEdit }: Props) {
  ...
  return (
    <div className="flex h-full flex-col gap-6 p-4 md:p-6 relative overflow-hidden min-h-screen">
      ...
      {/* Patient header card (Glassmorphism Premium Redesigned with Spacious Layout) */}
          ...
          <div className="space-y-4 min-w-0 flex-1">
            <div className="space-y-2">
              ...
              {patient.origem && (
                <p className="text-xs text-muted-foreground font-semibold flex items-center gap-1.5 mt-0.5">
                  ...
                  Origem do cadastro: <span className="text-foreground/80 font-bold capitalize">{patient.origem}</span>
                </p>
              )}
            </div>
            ...
            <div className="flex flex-wrap gap-2 pt-0.5">
              {patient.alergias.map(a => (
                <Badge key={a} className="bg-red-500/10 border border-red-500/20 text-red-600 dark:text-red-400 text-[10px] font-bold px-3.5 py-1 rounded-full shadow-sm flex items-center gap-1.5 select-none transition-colors hover:bg-red-500/15">
              ...
              {patient.alergias.length === 0 && patient.tags.length === 0 && (
                <Badge variant="secondary" className="text-[10px] font-semibold text-muted-foreground/80 bg-muted/40 border border-border/10 py-1 px-3.5 rounded-full select-none">
                  Sem alergias ou observações registradas
                </Badge>
              )}
            </div>
          </div>
