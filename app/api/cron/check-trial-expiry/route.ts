  ...
  try {
    ...
    for (const org of expiringSoon) {
      ...
      try {
        await sendHtmlEmail({
          to: owner.email,
          subject: 'Seu trial do Estetia CRM expira amanhã',
          html: `
            <p>Olá${owner.name ? `, ${owner.name}` : ''}!</p>
            <p>Seu trial PRO do Estetia CRM expira <strong>amanhã</strong>. Após isso, sua conta ficará em modo somente leitura.</p>
            <p><a href="https://estetiacrm.com.br/dashboard/billing">Fazer upgrade agora →</a></p>
          `,
        })
    ...
    for (const org of expired) {
      ...
      try {
        await sendHtmlEmail({
          to: owner.email,
          subject: 'Seu trial do Estetia CRM expirou',
          html: `
            <p>Olá${owner.name ? `, ${owner.name}` : ''}!</p>
            <p>Seu trial PRO do Estetia CRM expirou. Sua conta está agora em <strong>modo somente leitura</strong> — você pode ver todos os seus dados, mas não criar ou editar registros.</p>
            <p>Faça upgrade para continuar usando o Estetia sem restrições:</p>
            <p><a href="https://estetiacrm.com.br/dashboard/billing">Ver planos e fazer upgrade →</a></p>
          `,
        })
