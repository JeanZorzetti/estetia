...
async function sendPixRenewalEmail(
  ...
  const subjects: Record<string, string> = {
    reminder: `⏰ Sua assinatura Estetia CRM vence em 3 dias – renove via PIX`,
    due: `💳 Renovação do Estetia CRM – seu plano vence hoje`,
  ...
  await sendEmail({
    ...
    html: `
      <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto;">
        ...
        <p>A renovação da assinatura <strong>${org.name}</strong> no Estetia CRM
...
async function sendDowngradeEmail(org: { id: string; name: string }) {
  ...
  await sendEmail({
    to: owner.email,
    subject: `Acesso ao Estetia CRM limitado – renove para recuperar`,
    html: `
      <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto;">
