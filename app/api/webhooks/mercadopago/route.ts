...
async function processRecurringPayment(paymentId: string) {
  ...
  try {
    ...
    if (owner?.email) {
      ...
      await sendEmail({
        to: owner.email,
        subject: '✅ Renovação confirmada – Estetia CRM',
        react: PaymentConfirmationEmail({
...
async function handleFailedRecurringPayment(organizationId: string, payment: any) {
  ...
  if (owner?.email) {
    await sendEmail({
      to: owner.email,
      subject: isFinal
        ? `⚠️ Assinatura Estetia cancelada por falta de pagamento`
        : `❌ Falha no pagamento (${newAttempts}/${MAX_PAYMENT_ATTEMPTS}) – Estetia CRM`,
