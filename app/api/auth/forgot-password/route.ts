...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Build reset URL
    const baseUrl = process.env.NEXTAUTH_URL || process.env.NEXT_PUBLIC_APP_URL || 'https://estetiacrm.com.br'
    const resetUrl = `${baseUrl}/reset-password?token=${token}`

    // Send email
    if (process.env.RESEND_API_KEY) {
      await resend.emails.send({
        from: process.env.EMAIL_FROM || 'Estetia CRM <noreply@estetiacrm.com.br>',
        to: normalizedEmail,
        subject: 'Redefinir sua senha — Estetia CRM',
        html: `
          <!DOCTYPE html>
          <html>
          <head>
          ...
          <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
            <div style="text-align: center; margin-bottom: 30px;">
              <h1 style="color: #6366f1; margin: 0;">Estetia CRM</h1>
            </div>

            <div style="background: #f9fafb; border-radius: 8px; padding: 30px; margin-bottom: 20px;">
              <h2 style="margin-top: 0; color: #111827;">Redefinir sua senha</h2>
              <p>Ola${user.name ? `, ${user.name}` : ''}!</p>
              <p>Recebemos uma solicitacao para redefinir a senha da sua conta no Estetia CRM.</p>
            ...
            <div style="text-align: center; font-size: 12px; color: #9ca3af;">
              ...
              <p>&copy; ${new Date().getFullYear()} Estetia CRM - ROI Labs</p>
            </div>
          </body>
          </html>
        `,
