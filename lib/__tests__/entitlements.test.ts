/**
 * Testes para o sistema de entitlements (matriz legada por tier).
 *
 * A matriz é @deprecated para gating de features novas (billing modular),
 * mas continua valendo para limites quantitativos, trial e webhooks —
 * estes testes pinam os valores atuais para pegar drift acidental.
 */
...
describe('Entitlements System', () => {
  describe('PLAN_FEATURES', () => {
    ...
    it('should have correct FREE tier limits', () => {
      ...
      expect(PLAN_FEATURES.FREE.can_use_automation).toBe(false)
    })
    ...
    it('should have correct PRO tier limits', () => {
      ...
      expect(PLAN_FEATURES.PRO.can_use_team_reports).toBe(true)
    })
  ...
  describe('getLimit', () => {
    ...
    it('should return quantitative limits for STARTER and PRO', () => {
      expect(getLimit('STARTER', 'max_deals')).toBe(500)
      expect(getLimit('PRO', 'max_deals')).toBe(2500)
      expect(getLimit('PRO', 'max_users')).toBe(15)
    })

    it('should return -1 (unlimited) only for BUSINESS deals/contacts', () => {
  ...
  describe('canUseFeature', () => {
    ...
    it('should allow STARTER tier to use AGI and automation', () => {
      expect(canUseFeature('STARTER', 'can_use_agi')).toBe(true)
      expect(canUseFeature('STARTER', 'can_use_automation')).toBe(true)
    })

    it('should allow PRO tier to use automation and team reports', () => {
      expect(canUseFeature('PRO', 'can_use_automation')).toBe(true)
      expect(canUseFeature('PRO', 'can_use_team_reports')).toBe(true)
    })

    it('should reserve chat interface and round robin for BUSINESS', () => {
      expect(canUseFeature('PRO', 'can_use_chat_interface')).toBe(false)
      expect(canUseFeature('PRO', 'can_use_round_robin')).toBe(false)
      expect(canUseFeature('BUSINESS', 'can_use_chat_interface')).toBe(true)
      expect(canUseFeature('BUSINESS', 'can_use_round_robin')).toBe(true)
    })

    it('should accept feature names with or without can_use_ prefix', () => {
      expect(canUseFeature('BUSINESS', 'round_robin')).toBe(true)
      expect(canUseFeature('BUSINESS', 'can_use_round_robin')).toBe(true)
    })
  ...
  describe('Tier hierarchy', () => {
    ...
    it('should grow limits monotonically FREE → STARTER → PRO', () => {
  ...
  describe('PLAN_LIMITS (feature flags por tier)', () => {
    it('should gate apiAccess/webhooks at PRO+', () => {
      ...
      expect(PLAN_LIMITS.BUSINESS.features.webhooks).toBe(true)
    })
  })

  describe('getEntitlementsFromModules (billing modular)', () => {
    it('should map module slugs to entitlements', () => {
      const ent = getEntitlementsFromModules(['prontuario', 'recall', 'ia_pro'])
      expect(ent.canUseProntuario).toBe(true)
      expect(ent.canUseRecall).toBe(true)
      expect(ent.canUseGaleria).toBe(false)
      expect(ent.aiAgentsLimit).toBe(3)
      expect(ent.aiActionsLimit).toBe(1000)
    })

    it('should disable IA when no ia module active', () => {
      const ent = getEntitlementsFromModules([])
      expect(ent.aiAgentsLimit).toBe(0)
      expect(ent.aiActionsLimit).toBe(0)
    })
  })
})
