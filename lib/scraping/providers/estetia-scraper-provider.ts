/**
 * Estetia Scraping Server Provider
 *
...
const SCRAPER_URL = process.env.ESTETIA_SCRAPER_URL

export class EstetiaScraperProvider implements ScrapingProvider {
  name = 'ESTETIA_SCRAPER'
  ...
  async search(params: ScrapingSearchParams): Promise<ScrapingSearchResult> {
    if (!SCRAPER_URL) {
      logger.error({}, 'ESTETIA_SCRAPER_URL not configured')
      throw new Error('ESTETIA_SCRAPER_URL not configured')
    }

    logger.info({ url: SCRAPER_URL, query: params.query, city: params.city }, 'Estetia scraper search started')
    ...
    try {
      ...
      const leads: ScrapingLead[] = data.leads.map((l: any) => ({
        ...
        email: l.email,
        website: l.website,
        source: 'ESTETIA_SCRAPER',
      }))

      logger.info({ found: leads.length }, 'Estetia scraper search completed')
...
export const estetiaScraperProvider = new EstetiaScraperProvider()
