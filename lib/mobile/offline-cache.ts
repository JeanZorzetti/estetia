...
interface EstetiaDB extends DBSchema {
  contacts: {
    key: string
...
let dbPromise: Promise<IDBPDatabase<EstetiaDB>> | null = null

function getDB(): Promise<IDBPDatabase<EstetiaDB>> {
  if (typeof window === 'undefined') {
    return Promise.reject(new Error('IndexedDB not available on server'))
  }
  if (!dbPromise) {
    dbPromise = openDB<EstetiaDB>(DB_NAME, DB_VERSION, {
      upgrade(db) {
