...
export async function createSubscription(
  ...
  const planTitles: Record<string, string> = {
    STARTER: `Estetia CRM Starter - ${organizationName}`,
    PRO: `Estetia CRM Pro - ${organizationName}`,
    BUSINESS: `Estetia CRM Business - ${organizationName}`,
    FOUNDER_STARTER: `Estetia CRM Fundador Starter - ${organizationName}`,
    FOUNDER_PRO: `Estetia CRM Fundador Pro - ${organizationName}`,
    FOUNDER_BUSINESS: `Estetia CRM Fundador Business - ${organizationName}`,
  }
  ...
  try {
    const subscription = await getPreApprovalClient().create({
      body: {
        reason: planTitles[plan] || `Estetia CRM ${plan} - ${organizationName}`,
