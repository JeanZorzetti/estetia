...
export async function getDealWhatsAppMessages(dealId: string) {
    ...
    const contactIds = [...new Set(messages.map((m: typeof messages[number]) => m.contactId).filter(Boolean))] as string[]
    const contacts = contactIds.length > 0
        ...
        : []
    const contactMap = new Map(contacts.map((c: typeof contacts[number]) => [c.id, c]))
...
export async function getOrganizationWhatsAppMessages(organizationId: string, limit: number = 50) {
    ...
    const contactIds = [...new Set(messages.map((m: typeof messages[number]) => m.contactId).filter(Boolean))] as string[]
    const dealIds = [...new Set(messages.map((m: typeof messages[number]) => m.dealId).filter(Boolean))] as string[]
    ...
    const contactMap = new Map(contacts.map((c: typeof contacts[number]) => [c.id, c]))
    const dealMap = new Map(deals.map((d: typeof deals[number]) => [d.id, d]))
