...
export function autoFillROICalculator(context: AutoFillContext): AutoFillResult<ROICalculatorProps> {
  ...
  // Calculate Estetia cost (PRO plan = R$147/user, estimate based on team size)
  ...
  const withEstetia = 147 * teamMultiplier
  const monthlySavings = Math.max(0, currentCost - withEstetia)
  const annualROI = monthlySavings * 12
  const paybackPeriod = monthlySavings > 0 ? Math.ceil(withEstetia / monthlySavings) : 0
...
export function autoFillDealForm(context: AutoFillContext): AutoFillResult<DealFormProps> {
  ...
  if (context.entities.companies.length > 0) {
    prefill.title = `Implementação Estetia CRM - ${context.entities.companies[0]}`
    filledFields.push('title')
  } else if (context.dealContext?.company) {
    prefill.title = `Implementação Estetia CRM - ${context.dealContext.company}`
    filledFields.push('title')
  } else {
